...
function formatRange(lead: Lead): string {
  ...
  const startStr = start.toLocaleDateString("pt-PT", { day: "2-digit", month: "short" });
  if (!lead.end_date || lead.end_date === lead.start_date) return startStr;
  const end = new Date(lead.end_date + "T00:00:00");
...
function ShareBox({ path, onRevoke }: { path: string; onRevoke: () => void }) {
  ...
  return (
    <div className="mt-2 flex items-center gap-2 bg-[#f4dfc8] rounded-lg px-2.5 py-1.5 text-[12px]">
      <input
        readOnly
        value={path}
...
export default function CalendarClient({ leads, shares: initialShares }: { leads: Lead[]; shares: Share[] }) {
        ...
        <section key={key} className="mb-7">
          ...
          <div className="bg-white border border-[#d8dbd3] rounded-xl overflow-hidden divide-y divide-[#d8dbd3]">
            {monthLeads.map((lead) => {
              ...
              return (
                <div key={lead.id} className="p-3.5 flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    {share && <ShareBox path={pathFor(share.token)} onRevoke={() => revoke(share.token)} />}
                  </div>
                  {!share && (
                    <button
